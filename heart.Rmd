...
2. Because of the way the heart was read in, all the variables were read in as floats. 
3. The missing values were -9. So I replaced the -9 with NAs. 
```{r}
...
```
4. There was a lot of missing data in various variables, hence when all rows with NAs were omitted, only 1 row remained. Only one observation was used to fit the model. One observation cannot determin a 2D line. 
```{r}
nrow(na.omit(heart))
```
